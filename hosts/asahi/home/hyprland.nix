...
let
  ...
  p = with pkgs; {
    ...
    term = e kitty;
    wofi = e wofi;
    thunar = e xfce.thunar;
    firefox = "${f} run org.mozilla.firefox";
  };
in
{
  xdg.configFile."hypr/hyprland.conf".text = ''
    ...
    monitor=HDMI-A-1,1920x1080@143.98Hz,auto,1.25
    ...
    bindl = , switch:off:Apple SMC power/lid events, exec, hyprctl keyword monitor "eDP-1,preferred,auto,3"
    ...
    $chat = ${p.legcord}
    $terminal = ${p.term}
    $fileManager = ${p.thunar}
    $menu = ${p.wofi} --show drun
    $browser = ${p.firefox}
